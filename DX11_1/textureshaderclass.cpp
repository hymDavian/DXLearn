...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	/*采样器状态描述在此处设置，然后可以传递给像素着色器。 纹理采样器描述中最重要的元素是过滤器。 
	过滤器将决定如何使用或组合哪些像素来创建多边形面纹理的最终外观。 
	在此示例中，我使用 D3D11_FILTER_MIN_MAG_MIP_LINEAR，它在处理方面更昂贵，
	但提供了最佳的视觉效果。 它告诉采样器使用线性插值进行缩小、放大和 mip 级采样。
	AddressU 和 AddressV 设置为 Wrap，可确保坐标保持在 0.0f 和 1.0f 之间。 
	除此之外的任何东西都会环绕并放置在 0.0f 和 1.0f 之间。 采样器状态描述的所有其他设置均为默认值。*/
